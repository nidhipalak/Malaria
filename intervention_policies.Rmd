...
```{r}
complete_2_df_unnest =
  left_join(estimate_cases_deaths_df, commodities_df, by = c("country","year")) %>% 
  mutate(
    prevalence = cases_point / population_at_risk,
    prevalence_lower = cases_lower / population_at_risk,
    prevalence_upper = cases_upper / population_at_risk,
    mortality = deaths_point / population_at_risk,
    mortality_lower = deaths_lower / population_at_risk,
    mortality_upper = deaths_upper / population_at_risk
  )
```

What is the population malaria prevelance and mortality?
```{r}
ggplotly(ggplot(complete_2_df_unnest, aes(x = year, y = prevalence, color = country)) + geom_point() + geom_line())

ggplotly(ggplot(complete_2_df_unnest, aes(x = year, y = mortality, color = country)) + geom_point() + geom_line() )
```

Clean up data from STAT compiler household statistics
```{r}
household_stat_df =
  read_excel(
...
Does a household having an ITN improve prevalence/mortality of malaria?

```{r}
preventative_complete_df = 
household_stat_df %>% 
  ...
  right_join(complete_2_df_unnest, by = "country")
```
